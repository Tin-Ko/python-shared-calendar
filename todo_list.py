...
#########################################################
# 在這裡從 server 拿 pkl 檔，存在跟程式碼同樣的地方（資料夾）就好
...
def select(e):
    ...
    test_label.config(text=f'task id : {current_task} , count : {count}')
...
def double_click(e):
    ...
    print(todo_list.todo_list[int(selected_task)].finished)
...
def add_task():
    ...
    if member == 'shared':
        if not todo_list.occupied_dates[due_date]:
...
def edit_task():
    ...
    my_tree.item(current_task, text=current_task_text, values=(task_input.get(), member_input.get(), date_input.get_date(), current_task_finished))
    task_input.delete(0, END)
    member_input.delete(0, END)
    date_input.delete(0, END)
...
def save_task():
    with open('local_tasks.pkl', "wb") as f:
        pickle.dump(todo_list, f)
    s.send("save".encode('utf-8'))
    time.sleep(1)
    with open('local_tasks.pkl','rb') as file1: #計算檔案大小送出告知要收幾次
            i=0
            for line in file1:
                i+=1
            print(i)
            ...
            time.sleep(1)
    with open('local_tasks.pkl','rb') as file1: #送出檔案
        for line in file1:
            s.send(line)
            time.sleep(1)
def load_tasks():
    ...
    time.sleep(1)
    with open('local_tasks.pkl','wb') as file:
            count_line = int(s.recv(1024))
            print("server_save",count_line)
            for i in range(count_line):
                d = s.recv(1024)
                file.write(d)
            print("server_save")
    with open('local_tasks.pkl', 'rb') as f:
...
# add_sub_button = Button(frame_buttons, text='Add Sub Task', width=10, command=add_sub_task)
# add_sub_button.grid(row=2, column=0)
